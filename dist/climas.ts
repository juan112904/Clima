const gotas:number=500
const copos:number=200
const lluvia=document.querySelector('.lluvia')!
const nieve=document.querySelector('.nieve')!

function crearLluvia(metodo?:number){
    if(metodo==1){
        lluvia.innerHTML=''
    }else{
        for(let i:number=0; i<=gotas;i++){
            let svg:string='<svg  width="18" height="18" fill="#ffffff"> <path stroke="none" d="M 2.5,0 C 2.6949458,3.5392017 3.344765,20.524571 4.4494577,30.9559 5.7551357,42.666753 4.5915685,50 2.5,50 0.40843152,50 -0.75513565,42.666753 0.55054234,30.9559 1.655235,20.524571 2.3050542,3.5392017 2.5,0 Z"></path></svg>'
            let gota:HTMLElement=document.createElement('div')
...
function crearNieve(metodo?:number){
    if(metodo==1){
        nieve.innerHTML=''
    }else{
        for(let i:number=0;i<=copos;i++){
